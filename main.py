...
---
            ...
            for subclase in selected_subclases:
                ...
                fig2d.add_trace(go.Scatter(
                    ...
                    mode="markers",
                    marker=dict(size=6, color=color_map[subclase], symbol=symbol_map.get(subclase, "circle")),
            ...
            for subclase in selected_subclases:
                ...
                fig3d.add_trace(go.Scatter3d(
                    ...
                    mode='markers',
                    marker=dict(size=5, color=color_map[subclase]),
                    name=f"Subclase {subclase}",
                    text=sub_df[target_var],
                    hovertemplate="RA: %{x:.2f}<br>Dec: %{y:.2f}<br>Subclase: %{text}"
                ))
        ...
        def plot_tsne_and_boxplots(df, parent_col, selected_cols, level):
            ...
            if tsne1 not in df.columns or tsne2 not in df.columns:
                ...
                for parent in parents:
                    ...
                    if max_comp < 2:  # con 1 muestra o 1 feature útil, no hay nada que hacer
                        df.loc[data.index, tsne1] = np.nan
                        df.loc[data.index, tsne2] = np.nan
                        continue

                    pca = PCA(n_components=min(20, max_comp)).fit_transform(scaled)
